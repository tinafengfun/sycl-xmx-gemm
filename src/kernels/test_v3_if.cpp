// ==========================================================================
// V3: 4SG 2x2 with empty if-block, no prefetch call (独立二进制)
// 测试 if(kpf<K) 分支本身对编译器调度的影响
// ==========================================================================
...
int main(int argc, char* argv[]) {
            ...
            h.parallel_for(
                ...
                [=](sycl::nd_item<2> item) [[sycl::reqd_sub_group_size(SG_SZ)]] {
                    ...
                    for(int k=0;k<K;k+=32) {
                        int kpf=k+32;
                        if(kpf<K) {
                            // Empty — just the branch structure, no prefetch
                        }
    ...
    printf("V3 (empty if-block): M=%d N=%d K=%d  best=%.3f ms  avg=%.3f ms  TF=%.2f\n",
