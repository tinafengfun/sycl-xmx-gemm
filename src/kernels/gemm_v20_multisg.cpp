// ==========================================================================
// BF16 GEMM v20c — Multi-SG experiments
//
// Strategy: Multiple sub-groups per work-group, each with smaller tiles.
// Previous multi-SG (v4-v7) used TM=32,TN=64 and got 64T (register spill).
// New approach: Each SG uses 2x2 tiles (OUT=16x32) with 256 GRF,
// multiple SGs cooperate to cover larger output blocks.
//
// Configurations to test:
//   v20c1: 4 SGs per WG (2x2 layout), each SG has 2x2 tiles
//   v20c2: 8 SGs per WG (4x2 layout), each SG has 1x2 tiles
//   v20c3: 2 SGs per WG (2x1 layout), each SG has 2x4 tiles
//   v20c4: 8 SGs per WG (2x4 layout), each SG has 2x1 tiles
// ==========================================================================
...
// ==========================================================================
// Single-SG baseline for comparison
// ==========================================================================
template<int MT_M, int MT_N>
double bench_single_sg(sycl::queue& q, int M, int N, int K,
...
// ==========================================================================
// Multi-SG: Each SG handles a row-stripe of the output tile
// SG layout: N_SG sub-groups per work-group, arranged along M dimension
// Each SG computes MT_M × MT_N tiles independently (no SLM, no sharing)
// Work-group covers: WG_M = N_SG * MT_M * TM, WG_N = MT_N * TN
//
// v20c1: 4 SGs along M, each 2x2 tiles → WG covers 64x32 per WG
// v20c2: 8 SGs along M, each 2x2 tiles → WG covers 128x32 per WG
// v20c3: 4 SGs along M, each 4x2 tiles → WG covers 128x32 per WG
// ==========================================================================

template<int MT_M, int MT_N, int N_SG_M, int N_SG_N>
double bench_multi_sg(sycl::queue& q, int M, int N, int K,
                       int warmup, int iters, const char* tag) {
    // Each SG: MT_M*TM x MT_N*TN output
    // WG: (N_SG_M * MT_M * TM) x (N_SG_N * MT_N * TN)
    ...
    // Local size: N_SG_TOTAL sub-groups × SG_SZ threads per SG
    // ND-range: global = (wg_m * N_SG_TOTAL * SG_SZ, wg_n)
    //           local = (N_SG_TOTAL * SG_SZ, 1)
    ...
    auto launch=[&]() -> sycl::event {
        return q.submit([&](sycl::handler& h) {
            h.parallel_for(
                ...
                [=](sycl::nd_item<2> item) [[sycl::reqd_sub_group_size(SG_SZ)]] {
                    sycl::sub_group sg = item.get_sub_group();
                    int sg_id = sg.get_group_id()[0]; // 0..N_SG_TOTAL-1
                    int sg_m = sg_id / N_SG_N;        // SG row
                    int sg_n = sg_id % N_SG_N;        // SG column

                    int gm = item.get_group(0); // WG group
                    int gn = item.get_group(1);

                    // Compute this SG's output position
...
int main(int argc, char* argv[]) {
    ...
    printf("=== BF16 GEMM v20c — Multi-SG experiments ===\n");
    printf("Device: %s\n\n", q.get_device().get_info<sycl::info::device::name>().c_str());

    // All tests with M=8192, N=2048, K=4096 (sweet spot)
    int M=8192, N=2048, K=4096;

    printf("=== Single-SG baseline ===\n");
    bench_single_sg<4,4>(q,M,N,K,warmup,iters,"single_4x4_OUT32x64");
    bench_single_sg<4,2>(q,M,N,K,warmup,iters,"single_4x2_OUT32x32");
    bench_single_sg<2,4>(q,M,N,K,warmup,iters,"single_2x4_OUT16x64");
    bench_single_sg<2,2>(q,M,N,K,warmup,iters,"single_2x2_OUT16x32");

    printf("\n=== Multi-SG (independent SGs, no SLM) ===\n");
    // 2 SGs along M, each with 2x2 tiles → WG covers 32x32
    bench_multi_sg<2,2, 2,1>(q,M,N,K,warmup,iters,"2SG_2x2each_M_OUT32x32");
    // 2 SGs along N, each with 2x2 tiles → WG covers 16x64
    bench_multi_sg<2,2, 1,2>(q,M,N,K,warmup,iters,"2SG_2x2each_N_OUT16x64");
    // 4 SGs (2x2 grid), each with 2x2 tiles → WG covers 32x64
    bench_multi_sg<2,2, 2,2>(q,M,N,K,warmup,iters,"4SG_2x2each_OUT32x64");
    // 8 SGs along M, each with 2x2 tiles → WG covers 128x32
    bench_multi_sg<2,2, 4,1>(q,M,N,K,warmup,iters,"4SG_2x2each_M_OUT64x32");
    // 4 SGs along M, each with 4x2 tiles → WG covers 128x32
    bench_multi_sg<4,2, 2,1>(q,M,N,K,warmup,iters,"2SG_4x2each_OUT64x32");
    // 2 SGs along M, each with 4x4 tiles → WG covers 64x64
    bench_multi_sg<4,4, 2,1>(q,M,N,K,warmup,iters,"2SG_4x4each_OUT64x64");
    // 4 SGs along N, each with 4x4 tiles → WG covers 32x256
    bench_multi_sg<4,4, 1,4>(q,M,N,K,warmup,iters,"4SG_4x4each_N_OUT32x256");

    printf("\n=== N=4096 ===\n");
    bench_single_sg<4,4>(q,8192,4096,4096,warmup,iters,"single_4x4_N4096");
    bench_multi_sg<2,2, 2,2>(q,8192,4096,4096,warmup,iters,"4SG_2x2each_N4096");
    bench_multi_sg<4,4, 2,1>(q,8192,4096,4096,warmup,iters,"2SG_4x4each_N4096");
