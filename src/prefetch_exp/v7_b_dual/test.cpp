// V7: B-only dual-distance prefetch L1 — 同时发出 k+32 和 k+64 预取
// 猜想: 双距离覆盖更远，给cache更多预热时间
...
int main(int argc,char*argv[]){
    ...
    printf("V7 B-only dual pf L1 k+32+k+64: %s\n",q.get_device().get_info<sycl::info::device::name>().c_str());
        ...
        return q.submit([&](sycl::handler&h){
            ...
            [=](sycl::nd_item<2>item)[[sycl::reqd_sub_group_size(SG_SZ)]]{
                ...
                for(int k=0;k<K;k+=32){
                    // Dual distance: k+32 AND k+64
                    int kpf1=k+32,kpf2=k+64;
                    if(kpf1<K){
                        matrix::joint_matrix_prefetch<8,32>(sg,dBv+(size_t)(kpf1/2)*(N*2)+(size_t)(out_n)*2,N*2,matrix::layout::row_major,sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::prefetch_hint_L1});
                    }
                    if(kpf2<K){
                        matrix::joint_matrix_prefetch<8,32>(sg,dBv+(size_t)(kpf2/2)*(N*2)+(size_t)(out_n)*2,N*2,matrix::layout::row_major,sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::prefetch_hint_L1});
                    }
