// V5: B-only prefetch L2 k+32 — L2在XeCore内共享，4个SG共用一份B数据
// 猜想: L1 prefetch会让4个SG各存一份B，L2只存一份，减少带宽浪费
        ...
        return q.submit([&](sycl::handler&h){
            ...
            [=](sycl::nd_item<2>item)[[sycl::reqd_sub_group_size(SG_SZ)]]{
                ...
                for(int k=0;k<K;k+=32){
                    // B prefetch with L2 hint instead of L1
                    int kpf=k+32;
                    if(kpf<K){
                        matrix::joint_matrix_prefetch<8,32>(sg,dBv+(size_t)(kpf/2)*(N*2)+(size_t)(out_n)*2,N*2,matrix::layout::row_major,sycl::ext::oneapi::experimental::properties{sycl::ext::oneapi::experimental::prefetch_hint_L2});
                    }
