// V4: B-only prefetch L1 k+32 — 去掉A prefetch的地址计算开销
// 猜想: A prefetch被编译器消除但地址计算指令仍存在(45条)，去掉后减少指令数
